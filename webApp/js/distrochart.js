...
function makeDistroChart(settings) {
	...
	chart.settings.data.map(function(d){d[chart.settings.yName]=+d[chart.settings.yName]})
    ...
    !function prepareData() {
		...
		if(chart.settings.Xdomain){
			chart.settings.Xdomain.map(function(d){
				chart.groupObjs[d]={};
				chart.groupObjs[d].values = [];
			});
		}
    ...
    !function prepareSettings() {
        ...
        chart.objs.yAxis = d3.axisLeft(chart.yScale)
			...
			.ticks(chart.yScale.ticks().length*chart.settings.yTicks);
    ...
    chart.update = function () {
		...
		chart.objs.g.select('.y.axis').call(chart.objs.yAxis.tickSizeInner(-chart.divWidth));
    ...
    !function prepareChart() {
        ...
        chart.objs.g.yAxis =chart.objs.axes.append("g")
            .attr("class", "y axis")
            .call(chart.objs.yAxis);
		
        chart.objs.g.append("text")
            .attr("class", "label")
		...
		//reorder svg element
		chart.objs.g.yAxis.node().appendChild(chart.objs.g.yAxis.select("path").node());
        
		// Create tooltip div
        chart.objs.tooltip = d3.select("body").append('div').attr('class', 'tooltip')
			.style("display","none")
        for (var cName in chart.groupObjs) {
     ...
     * @param [options.width=50] The max percent of the group bandwidth that the violin can be
    ...
    chart.renderViolinPlot = function (options) {
        ...
        chart.violinPlots.update = function () {
            ...
            for (cName in chart.groupObjs) {
                ...
                cViolinPlot.kde = kernelDensityEstimator(eKernel(calcBandwith(chart.range)), xVScale.ticks(vOpts.resolution));
                ...
                if (vOpts.clamp == 0 || vOpts.clamp == -1) { //
                    ...
                    if (vOpts.clamp == -1) {
                        kdeTester = eKernelTest(eKernel(calcBandwith(chart.range)), chart.groupObjs[cName].values);
                        if (!interpolateMax) {
        ...
        function eKernel(scale) {
            return function (u) {
                return Math.abs(u /= scale) <= .75 ? .75 * (1 - u * u) / scale : 0;
            };
        }
     ...
     * @param [options.boxwidth=30] The max percent of the group bandwidth that the box can be
     * @param [options.lineWidth=boxWidth] The max percent of the group bandwidth that the line can be
    ...
    chart.renderBoxPlot = function (options) {
        ...
        chart.boxPlots.update = function () {
            ...
            for (cName in chart.groupObjs) {
                ...
                var pt;
                if (chart.settings.showOutliers) {
                ...
                if (!chart.settings.constrainExtremes) {
...
function calcBandwith(range){
	return(range[range.length-1]/40)
}